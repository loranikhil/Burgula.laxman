...
const FloatingActions = ({ 
  ...
  youtubeUrl = "https://m.youtube.com/@neetv2531",
  scrollThreshold = 300,
  ...
  return (
    <div className={`floating-actions-wrapper ${floatActionsVisible ? 'floating-actions-visible' : ''} floating-actions-${position}`}>
      ...
      <div 
        className={`floating-action-btn floating-youtube-btn ${showPulse ? 'floating-pulse-animation' : ''}`}
        onClick={handleYouTubeClick}
        onMouseEnter={() => setFloatButtonHovered('youtube')}
        onMouseLeave={() => setFloatButtonHovered(null)}
        title="Visit YouTube"
        style={{ animationDelay: `${animationDelay + 0.4}s` }}
      >
        <YouTubeIcon size={24} />
        <span className="floating-btn-tooltip">YouTube</span>
        <div className="floating-ripple-effect"></div>
      </div>
    </div>
  );
